...
unsigned long int value = 0; // to send to check_Inst()
...
// Setting our flags back to default values
  flag = 0;
  flagStop = 0;
...
void go_Forward_Seq(int j) {
  ...
  movement_Inst_Fwd();
  Serial.println(total_Fwd_Time[j]);
...
void go_Left_Seq(int k) {
  ...
  movement_Inst_Lft();
  Serial.println(total_Lft_Time[k]);
...
void go_Right_Seq(int l) {
  ...
  movement_Inst_Rgt();
  Serial.println(total_Rgt_Time[l]);
...
void go_Backward_Seq(int m) {
  ...
  movement_Inst_Bwd();
  Serial.println(total_Bwd_Time[m]);
...
void go_Stop_Seq(int n) {
  ...
  movement_Inst_Stp();
  Serial.println(total_Stp_Time[n]);
...
char curr_receivedChar;
char received_char = 'x';

void loop(){
  curr_receivedChar = (char)SerialBT.read();
...
//    delay(50);
  ...
  if(SerialBT.available()) {
    ...
    if(received_char != curr_receivedChar)
    {
         received_char = curr_receivedChar;
